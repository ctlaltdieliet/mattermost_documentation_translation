Nalevingsrapportage en Oversight (E20)
-----------------------------------------

Beschikbaar in ' Enterprise Edition E20 <https://mattermost.com/pricing-self-managed/> ` __. .. Opmerking:
  Deze functie wordt vervangen door een nieuwe :doc: ` Compliance Export feature <compliance-export>` en wordt verwijderd in Matterbest v6.0. Wij raden u aan te migreren naar het nieuwe systeem. Voor een voorbeeld-CSV-uitvoer van het nieuwe nalevingsexportsysteem, ` download hier een CSV-exportbestand <https://github.com/mattermost/docs/blob/master/source/samples/csv_export.zip> ` __.
...
Standaard bewaren alle Mattermeeste Editions alle berichten, inclusief bewerkingen en wisbewerkingen, samen met alle geüploade bestanden. 

Nalevingsrapportage inschakelen == == == == == == == == == == == == == == =

Na aankoop en installatie van een licentiesleutel voor Enterprise Edition E20: 

1. Ga naar ** System Console > Compliance > Compliance Monitoring * * (of ** System Console > General > Compliance > Enable Compliance * * in versies prior to 5.12) en stel de waarde van ** Enable Compliance Reporting * * in op ** true**.
...
Dagelijkse nalevingsrapporten == == == == == == == == == == == == == == == == ==

Na het inschakelen van nalevingsrapportage: 

1. Ga naar ** System Console > Compliance > Compliance Monitoring * * (of ** System Console > General > Compliance > Enable Daily Report * * in versies ouder dan 5.12) en stel de waarde van ** Enable Daily Report * * in op ** true**.
2. Klik op ** Save**. 
...
1. Ga naar ** System Console > > Compliance > Compliance Monitoring * * (of ** System Console > Compliance > Compliance Monitoring * * in versies prior to 5.12).
2. Vul de volgende criteria in:  

     -** Taaknaam: ** Naam het nalevingsrapport dat u gaat uitvoeren (bijvoorbeeld "HR Audit 455").
     -** From:** Begindatum voor
zoeken in de notatie JJJJ-MM-DD (bv. "2016-03-11").
     ...
     -** Sleutelwerk:** Geef de woorden aan die in een bericht zouden worden opgenomen om te worden opgenomen in de resultaten van het nalevingsrapport. 3. Klik op ** Nalevingsrapport uitvoeren * *.
...
-** Timestamp:** Tijd waarop het rapport is aangevraagd.-** Status:** ` ` running ` ` geeft aan dat het rapport wordt uitgevoerd; ` ` voltooid ` ` geeft aan dat het rapport voltooid is en klaar is om te downloaden.
-** Records:** Geeft het aantal zoekresultaten weer.
-** Type:** ` ` adhoc ` ` geeft aan dat het rapport is aangevraagd door queryvelden in te vullen; ` ` daily ` ` geeft aan dat het rapport een dagelijkse export is.-** Beschrijving:** Taaknaam aangegeven in aanvraag.
...
| Chann
elName | URL-naam van kanaal | community-heartbeat | + --------------------- + --------------------------------------------------------------- + ------------------------------- +
| UserUsername | Gebruikersnaam van gebruiker die het sleutelwoord heeft gepost | frank.yu | + --------------------- + --------------------------------------------------------------- + ------------------------------- +
...
U kunt ook de nieuwe :doc: ` Compliance Export feature <compliance-export>` gebruiken voor de export van Global Relay.
