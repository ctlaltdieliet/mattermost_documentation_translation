...
Matterbest ondersteunt officieel Okta, OneLogin en Microsoft ADFS als de identiteitsproviders (IDPs), zie de links hieronder voor meer informatie over het configureren van SAML met deze providers. .. tocboom::
  :titlesonly:

  Okta SAML-configuratie <sso-saml-okta>
  OneLogin SAML-configuratie <sso-saml-onelogin.rst>
...
2. Instellen ** Schakel beheerderskenmerk * * in op ` ` true ` ` `.
...
Vanuit Matterbest v5.14 kunt u desgewenst Matterbest configureren voor het ondertekenen van de SAML-aanvraag met behulp van een persoonlijke sleutel om te voldoen aan de InfoSec-vereisten van uw organisatie.

In de toekomstige routekaart is de belangrijkste overweging een integratie met SCIM, via de plugin. Een dergelijke integratie stelt systeembeheerders in staat om SAML-provisioned gebruikers te maken voor hun eerste aanmelding, en synchroniseer ze tegen Mattermeeste permissies.

Momenteel kunnen gebruikersconfiguratie en deprovisioning worden afgehandeld met SAML-synchronisatie, maar is afhankelijk van AD/LDAP. Met SCIM kunnen beheerders gebruikersconfiguratie en deprovisioning binnen de IdP zelf beheren.

Zie voor voorbeelden ` Microsoft Azure AD integration with SCIM <https://docs.microsoft.com/en-us/azure/active-directory/manage-apps/use-scim-to-provision-users-and-groups> ` _ en ` Okta user provisioning with SCIM <https://www.okta.com/integrate/documentation/scim/> ` _.
...
Voor technische documentatie over SAML, zie :doc: ` sso-saml-technical `.
